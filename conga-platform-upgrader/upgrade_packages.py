...
def _read_nuget_creds(source_url: str) -> tuple[str, str] | None:
    """Read username + ClearTextPassword for *source_url* from NuGet.Config.

    Checks %APPDATA%/NuGet/NuGet.Config then the solution-local NuGet.Config.
    Returns (username, password) or None if not found.
    """
    import xml.etree.ElementTree as ET, os
    candidates = [
        Path(os.environ.get("APPDATA", "")) / "NuGet" / "NuGet.Config",
        SCRIPT_DIR.parent.parent / "NuGet.Config"  # copilot-skills/../NuGet.Config (solution root),
    ]
...
def cmd_find_revenue_version(args):
    ...
    """
    source       = args.source
    creds        = (args.username, args.password) if args.username else _read_nuget_creds(source)
